...
`subscription_mappings` 是**唯一的推送目标配置**，用 `*` 通配符控制"全局广播"还是"按博主订阅"。
所有推送——微博动态、热搜榜、每日总结——都通过这一个配置项决定发给谁。
...
|-----------|--------|
| 一个群接收所有博主 | `群ID: *` |
| 一个群只接收某个博主 | `群ID: 博主UID` |
| 两个群各自收不同博主 | 两行，每行指定不同 UID |
| 大群收全部，小群只收精选 | 大群用 `*`，小群指定 UID |

**关键点**：
- `*` = 我把所有监控博主 + 热搜 + 每日总结都发到这个会话
- 指定 UID = 我只把这些博主的微博发到这个会话（热搜和总结不发）
...
> 小明的群会收到李佳琦每条微博 + 罗永浩每条微博 + 热搜榜 + 每日总结。简单粗暴。
...
升级后，如果你想让某个原本"只收订阅"的群也收热搜和总结，把那一行的 UID 改成 `*` 即可。
